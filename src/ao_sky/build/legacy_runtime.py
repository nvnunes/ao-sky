"""Temporary legacy-derived runtime policy used by migration-phase builds."""
...
def load_legacy_runtime(
    ...
    """Load the temporary migration-phase AO/runtime policy from legacy config."""
...
def _to_persisted_asterisms_from_legacy(asterisms: Table | None) -> Table:
    if asterisms is None or len(asterisms) == 0:
        return _empty_persisted_asterisms()

    result = Table()
    result["asterism_id"] = np.asarray(asterisms["id"], dtype=np.int64)
    ...
    for index in (1, 2, 3):
        result[f"star{index}_source_id"] = np.asarray(asterisms[f"star{index}_id"], dtype=np.int64)
...
def load_live_legacy_traversal_outputs(
    ...
    This is a temporary migration helper used both by the Phase 5 build path
    and by the live legacy-comparison harness. The returned asterism table uses
    the legacy retained-catalog schema when present; the inner table already
    uses the cleaned Phase 5 field names.
    """
...
def build_traversal_products(
    ...
    Phase 5 keeps the new persisted contract and state model in ``ao-sky``, but
    still relies on the live legacy aomap runtime for the richer per-pixel
    metrics until the local AO model path is rebuilt. The canonical raw Gaia
    store remains the long-term source of truth, but this temporary adapter
    intentionally reuses the survey_tools Gaia cache that already exists for the
